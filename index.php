...
if($method == 'GET') {
  ...
  if($job == 'get')
  {
    ...
    echo json_encode($result->fetch());
    //echo json_encode($result->errorInfo());
  }
  ...
  else if($job == 'download')
  {
    ...
    {
        fputcsv($output, array('Average of each team'));
        fputcsv($output, array('Title','Session','Technical Accuracy','Creativity and Innovation','Supporting Analytical Work','Methodical Design Process Demonstrated','Addresses Project Complexity Appropriately','Expectation of Completion (by term’s end)','Design & Analysis of tests','Quality of Response during Q&A','Organization','Use of Allotted Time','Visual Aids','Confidence and Poise','Grand Total'));
        $current = $each;
        $counter = 1;
           while($each = $summary->fetch()){
            ...
            $titleSQL = "select * from projects where code='".$current['code']."'";
            $titleResult = $dbh->query($titleSQL);
            $titleRecord = $titleResult->fetch();
            $current['code'] = $titleRecord['Title'];
          ...
          fputcsv($output, array('Details of each team'));
          fputcsv($output, array('Title','Session','Judge','Technical Accuracy','Creativity and Innovation','Supporting Analytical Work','Methodical Design Process Demonstrated','Addresses Project Complexity Appropriately','Expectation of Completion (by term’s end)','Design & Analysis of tests','Quality of Response during Q&A','Organization','Use of Allotted Time','Visual Aids','Confidence and Poise','Grand Total','Economic','Environmental','Sustainability','Manufacturability','Ethical','Health and Safety','Social','Political','Comments'));
          ...
          while($row = $result->fetch()){
            ...
            $titleSQL = "select * from projects where code='".$row['code']."'";
            $titleResult = $dbh->query($titleSQL);
            $titleRecord = $titleResult->fetch();
            $row['code'] = $titleRecord['Title'];
            fputcsv($output, $row);
          }
